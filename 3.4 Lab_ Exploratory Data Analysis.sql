...
SELECT totalQuantity, alpha3Code FROM salesQuants s 
INNER JOIN countryCodes c on c.EnglishShortName = s.countryName
...
SELECT DISTINCT countryName FROM outdoorproducts;
...
SELECT totalQuantity, alpha3Code FROM salesQuants s 
INNER JOIN modCountryCodes m on m.EnglishShortName = s.countryName
