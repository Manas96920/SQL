...
SELECT *,
exists(batteryLevel, value -> value < 0) AS needService FROM DcDevices
...
timeStamp AS time FROM DcDevices)
...
SELECT batteryLevel,
  ...
  p_deviceID,
  deviceType,
  signal,time,reduce(temps, 0, (t, acc) -> t+acc, acc -> (acc DIV size(temps))) AS avg_daily_temps_c 
FROM parTable
...
CREATE WIDGET DROPDOWN selectedDeviceType DEFAULT "sensor-inest" CHOICES SELECT DISTINCT deviceType FROM parTable;
